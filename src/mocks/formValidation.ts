...
export function getMessageError(input: any): string {
  const error = input.split('"');
  switch (error[1]) {
    case "email":
      return "email đã được sử dụng.";
    case "cardId":
      if (error[2].indexOf("invalid") > 0) return "CMND không hợp lệ.";
...
export function handleDate(input: any): string {
  const output = moment(String(input)).format("DD/MM/YYYY");
  return output;
}
export function unHandleDate(input: any): string {
  const date = input.split("/");
  const output = `${date[2]}/${date[1]}/${date[0]}`;
  return output;
}
export function handleSuperInfoDate(input: any): string {
  const output = moment(String(input))
    .format("DD táng MM,YYYY")
    .replace("t", "th");
  return output;
}
export function compareNowDate(input: any): boolean {
  const date = new Date(unHandleDate(input));
  const nowDate = new Date();
  return date < nowDate;
}

export const handleValidation: any = {
  reunicode,
  handleDate,
  unHandleDate,
  handleSuperInfoDate,
